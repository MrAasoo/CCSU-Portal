...
import com.college.portal.modules.classroom.adapter.MessageAdapter;
import com.college.portal.modules.classroom.module.Chats;
...
public class ClassroomMessagesActivity extends AppCompat {
    ...
    //user data
    SharedPrefManager prefManager = new SharedPrefManager(ClassroomMessagesActivity.this);
    //Views
    private EditText textMessage;
    private ImageButton btnSend;
    private RecyclerView recyclerView;
    private MessageAdapter mAdapter;
    private List<Chats> mList;
    //Firebase
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle(getIntent().getStringExtra(AppApi.ROOM_NAME));
                ...
                if (textMessage.getText().toString().trim().isEmpty()) {
                    ...
                    if (prefManager.isLoggedIn()) {
                        sendMessage(textMessage.getText().toString().trim(),
                                ...
                                getIntent().getStringExtra(AppApi.ROOM_KEY));
                    } else {
                        Toast.makeText(ClassroomMessagesActivity.this, "Can't send message.", Toast.LENGTH_SHORT).show();
                        btnSend.setClickable(true);
                    }
                }
            }
        ...
        recyclerView.setLayoutManager(new LinearLayoutManager(ClassroomMessagesActivity.this));
        recyclerView.setHasFixedSize(true);
        mList = new ArrayList<>();
        mAdapter = new MessageAdapter(ClassroomMessagesActivity.this, mList);
    ...
    private void updateFirebaseUser(FirebaseUser user) {
        ...
        StudentPref prefManager = SharedPrefManager.getInstance(ClassroomMessagesActivity.this).getStudentInfo();
                    ...
                    .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if (task.isSuccessful()) {
                                Toast.makeText(ClassroomMessagesActivity.this, "Successful", Toast.LENGTH_SHORT).show();
                                mUser = mAuth.getCurrentUser();
                            } else {
                                Toast.makeText(ClassroomMessagesActivity.this, "Failed", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
        }
    ...
    private void getClassroomChatData() {
        ...
        reference.child("classroom_chats").child(getIntent().getStringExtra(AppApi.ROOM_KEY)).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                mList.clear();
                for (DataSnapshot data : snapshot.getChildren()) {

                    Chats chat = data.getValue(Chats.class);
    ...
    private void sendMessage(String message, String stdId, String stdName, String roomKey) {
        ...
        reference.child("classroom_chats").child(roomKey).push().setValue(map).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    ...
                    Toast.makeText(ClassroomMessagesActivity.this, "Message not sent", Toast.LENGTH_SHORT).show();
                    btnSend.setClickable(true);
                }
            }
        });
